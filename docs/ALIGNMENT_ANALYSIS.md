...
| Q13 | ✅ 钩子 = Python Callable (机制), 接线由 Agent config 决定 (策略)。**实现 defer** | python-sdk-agent InferenceMux 钩子点; Phase 1 不做钩子 |
| Q14 | ✅ Agent = 多服务器 MCP Client, 统一 notification buffer。**实现部分 defer** | 架构确认; Phase 1 做 notification buffer, 钩子/trace defer |
...
**方向确认**: SDK 提供事件钩子 (Python Callable)。接线由 Agent config 决定 (发 MCP Notification / 忽略 / 自定义)。

**实现 defer**: Phase 1 先不做钩子系统。需要想清楚 DX (config-driven vs code-driven)，避免 "Agent 要动手写代码自己用 SDK" 的别扭。

**哲学依据**: P4 钩子是机制, 接线是策略; P3 OS 不硬编码应用路径

### Q14: MCP Notification 替代 Mailbox ✅ (方向确认, 实现部分 defer)

**方向确认**:

1. **Agent = 多服务器 MCP Client**, 维持多个持久连接:
   - 自己的 MCPServerComponent (agent-local tools)
   - TagentacleMCPServer (内核接口, daemon 级操作)
   - shell-server, weather-server 等

2. **统一 notification buffer**: 所有 MCP Server 的 Notification 流入同一 buffer

3. **MessageQueue 被替代**: MCP notification buffer 取代自定义 MessageQueue

4. **底层传输分流** (按 Q12 标准):
   - 广播事件 (log, 节点事件) → bus topic → MCPServerComponent 转为 notification
   - 定向消息 (subscribed topic 到达) → MCP notification 直发
   - Agent 不关心底层传输 = MCP 抽象层价值

5. **Phase 1 实现范围**:
   - notification buffer 架构 ✅
   - InferenceMux 输入 = notification batch ✅
   - 钩子/trace 系统 defer ❌

**哲学依据**: P5 端到端 (流式直连); P4 两种传输都提供不强制; P3 MCP 为域 Shell
